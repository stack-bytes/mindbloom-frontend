      ...
      (async () => {
        try {
          const response = await fetch(
            `http://rnjks-5-2-197-133.a.free.pinggy.link/core-services/users/?userId=${user.userId}`,
            {
              method: "GET",
              headers: {
      ...
      <TouchableOpacity
        onPress={() => {
          ...
          for (let i = 0; i < deletedInterests.length; i++) {
            (async () => {
              const url = new URL(
                "http://rnjks-5-2-197-133.a.free.pinggy.link/core-services/users/interests"
              );
