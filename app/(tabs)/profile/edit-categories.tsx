      ...
      <View className="flex-1 items-center justify-center bg-gray-50 px-6">
        ...
        <View className="mb-6 w-full max-w-md rounded-lg border border-gray-300 bg-white p-4 shadow-md">
          ...
          <TouchableOpacity
            onPress={addInterest}
            style={{
              backgroundColor: NAV_THEME.light.primary,
              paddingVertical: 10,
              borderRadius: 25,
              alignItems: "center",
            }}
        ...
        <TouchableOpacity
          onPress={() => {
            if (anotherInterests.length > 0) {
              (async () => {
                const url = new URL(
                  `${ENDPOINT}/core-services/users/interests`
                );
            ...
            router.push(`/profile/${localUser.userId}`);
          }}
          style={{
            backgroundColor: NAV_THEME.light.primary,
            paddingVertical: 12,
        ...
        >
          <Text className="text-sm font-medium text-white">Apply changes</Text>
        </TouchableOpacity>
      </View>
    </TouchableWithoutFeedback>
  );
